...
sub get_network_address
{
    ...
    return inet_ntoa(pack 'N', (unpack 'N', inet_aton($address)) & $netmask);
}
...
=item $prefix = $t->get_prefix_from_netmask($numeric_netmask)
...
Returns the base network address from a host address and a netmask (given
as either prefix length of netmask in numeric IPv4 format).
