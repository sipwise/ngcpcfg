...
@pytest.mark.tt_16903
def test_set_action_generate_dict_digit_no_diff(ngcpcfgcli, tmpdir):
    tmpfile = tmpdir.join("tmpfile.txt")
    tmpfile.write("---\n")
...
@pytest.mark.tt_51601
def test_set_action_generate_dict_digit_diff(ngcpcfgcli, cli, tmpdir, gitrepo):
...
'''
    assert "" in out.stdout
    assert "Key resolved to a ARRAY reference; refusing to overwrite." \
        in out.stderr
    assert out.returncode == 1
...
'''
    assert "" in out.stdout
    assert "Key resolved to a HASH reference; refusing to overwrite." \
        in out.stderr
    assert out.returncode == 1
