...
"""
    ...
    generated_customtt = (
        str(tmpdir) + "/etc/apt/apt.conf.d/71_no_recommended.customtt.tt2"
    )
...
"""
    ...
    assert (
        "Error: The patch '"
        + str(template_path)
        + "/apt/apt.conf.d/71_no_recommended.patchtt.tt2' cannot be applied"
        in out.stderr
    )
    assert (
...
"""
    ...
    assert (
        ...
        + str(template_path)
        + "/apt/apt.conf.d/71_no_recommended.tt2'"
        in out.stderr
    )
    assert (
        "Error: Some operations above finished with an error for the file(s)"
...
"""
    ...
    assert (
        "Validating patch '"
        + str(template_path)
        + "/apt/apt.conf.d/71_no_recommended.patchtt.tt2.sp1'"
        in out.stdout
    )
    assert (
        "Applying patch '"
    ...
    generated_customtt = (
        str(template_path) + "/apt/apt.conf.d/71_no_recommended.customtt.tt2"
    )
    ...
    generated_customtt_sp1 = (
        str(template_path) + "/apt/apt.conf.d/71_no_recommended.customtt.tt2.sp1"
    )
    ...
    generated_config = (
        str(out.outdir) + str(tmpdir) + "/etc/apt/apt.conf.d/71_no_recommended"
    )
...
"""
    ...
    assert (
        "Validating patch '" + str(template_path) + "/dummy/dummy.patchtt.tt2'"
        in out.stdout
    )
    assert (
        "Applying patch '"
    ...
    assert (
        "Applying patch '" + str(template_path) + "/dummy/dummy.patchtt.tt2'"
        in out.stdout
    )
    assert (
        "Successfully created '" + str(template_path) + "/dummy/dummy.customtt.tt2'"
        in out.stdout
    )
    assert "Requested patchtt operation has finished successfully." in out.stdout
    assert (
...
"""
    ...
    template_path.join("expected_patch.diff").write(
        """@@ -1,3 +1,3 @@
...
"""
    ...
    generated_patchtt = (
        str(template_path) + "/apt/apt.conf.d/71_no_recommended.patchtt.tt2"
    )
    expected_patchtt = str(template_path) + "/expected_patch.diff"
